  ...
  return (
    <nav style={{
      ...
      <button onClick={() => window.scrollTo({ top: 0, behavior: "smooth" })} className="clickable"
        ...
        <div style={{ width: 28, height: 28, borderRadius: "7px", background: "var(--text)", display: "flex", alignItems: "center", justifyContent: "center" }}>
          <span style={{ fontSize: "0.7rem", fontWeight: 800, color: "var(--bg)", letterSpacing: "-0.04em" }}>SM</span>
        </div>
        <span style={{ fontSize: "0.9rem", fontWeight: 600, color: "var(--text)", opacity: scrolled ? 1 : 0, transition: "opacity 0.4s" }}>Sanjit Mathur</span>
      </button>

      <div style={{ display: "flex", gap: "0.25rem", background: scrolled ? "rgba(15,13,10,0.04)" : "transparent", padding: scrolled ? "0.3rem" : "0", borderRadius: "10px", border: scrolled ? "1px solid var(--border)" : "none", transition: "all 0.4s" }}>
        {sections.map(s => (
          <button key={s} onClick={() => go(s)} className="clickable"
            style={{ background: active === s ? "rgba(15,13,10,0.07)" : "transparent", border: "none", cursor: "none", padding: "0.35rem 0.85rem", borderRadius: "7px", fontSize: "0.82rem", fontWeight: active === s ? 600 : 400, color: active === s ? "var(--text)" : "var(--muted)", transition: "all 0.2s" }}>
            {labels[s]}
          </button>
        ))}
      </div>
