...
export default function IndiGoWidget() {
  ...
  // Flight arc path (cubic bezier) from DEL to BOM
  const W = 260, H = 160;
  const DEL = { x: 120, y: 45 };  // Delhi
  const BOM = { x: 70, y: 115 };  // Mumbai
  ...
  // Donut ring for 88% accuracy
  const r = 28, cx = 210, cy = 80;
  const circ = 2 * Math.PI * r;
  const dashOffset = circ * (1 - accuracy / 100);

  return (
    <div style={{ width: "100%", height: "100%", background: "#060a14", borderRadius: 12, padding: "14px 16px", fontFamily: "var(--font)", overflow: "hidden", position: "relative" }}>
      {/* Header */}
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 6 }}>
        ...
        <div style={{ fontSize: "0.5rem", color: "#6366f1", background: "rgba(99,102,241,0.12)", padding: "2px 7px", borderRadius: 100, border: "1px solid rgba(99,102,241,0.2)" }}>
          Logistic Regression
        </div>
      </div>
      ...
      <svg width="100%" height={H} viewBox={`0 0 ${W} ${H}`}>
        ...
        {Array.from({ length: 6 }, (_, row) =>
          Array.from({ length: 9 }, (_, col) => (
            <circle key={`${row}-${col}`} cx={col * 30 + 10} cy={row * 28 + 8} r={0.8} fill="rgba(255,255,255,0.06)" />
          ))
        )}

        {/* India outline simplified */}
        <path d="M85,20 L130,15 L160,30 L165,55 L155,80 L140,100 L120,120 L100,135 L80,130 L65,110 L60,85 L65,60 L75,40 Z"
          fill="rgba(99,102,241,0.06)" stroke="rgba(99,102,241,0.2)" strokeWidth="0.8" />

        {/* Flight arc dashed */}
        <path
          d={`M${DEL.x},${DEL.y} C${CP1.x},${CP1.y} ${CP2.x},${CP2.y} ${BOM.x},${BOM.y}`}
          fill="none" stroke="rgba(99,102,241,0.35)" strokeWidth="1" strokeDasharray="4 3"
        />
        {/* Completed portion */}
        <path
          d={`M${DEL.x},${DEL.y} C${CP1.x},${CP1.y} ${CP2.x},${CP2.y} ${BOM.x},${BOM.y}`}
          fill="none" stroke="#6366f1" strokeWidth="1.5" strokeDasharray={`${progress * 200} 1000`}
          strokeLinecap="round"
        />

        {/* City dots */}
        <circle cx={DEL.x} cy={DEL.y} r={4} fill="#6366f1" />
        <circle cx={DEL.x} cy={DEL.y} r={7} fill="none" stroke="rgba(99,102,241,0.4)" strokeWidth="1" />
