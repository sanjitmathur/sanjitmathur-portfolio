...
def extract_airplane_frames(
    video_path="Airplane_flying_across_dark_sky_202609081202.mp4",
    output_dir="artifacts/portfolio/public/airplane-frames",
    target_width=1280,
    target_height=720,
    total_frames=120,
    quality=82
    ...
    print(f"Video opened: {orig_w}x{orig_h}, {total_video_frames} frames")
    ...
    lead_in = 6
    lead_out = 8
    mid_count = total_frames - lead_in - lead_out # 106 frames

    frames_written = 0

    # 1. Lead-in frames (plane entering from off-screen left)
    for i in range(lead_in):
        # Shift amount from -450px down to -75px
    ...
    # 2. Main video frames (sampled from video frames 1 to 238)
    # Preload video frames or seek
    ...
    # 3. Lead-out frames (plane gliding fully off-screen right)
